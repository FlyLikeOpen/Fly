...
namespace Fly.Framework.Common
{
    public class RestClient
    {
        ...
        protected virtual object DeserializeResponseData(string responseTxt, RequestFormat format, Type type)
        {
            ...
            switch (format)
            {
                case RequestFormat.Json:
                    return Serialization.JsonDeserialize(responseTxt, type);
                case RequestFormat.Xml:
                    return Serialization.XmlDeserialize(responseTxt, type);
                case RequestFormat.Raw:
                    return responseTxt;
                default:
                    return Serialization.BinaryDeserialize(responseTxt);
            }
        }
        ...
        protected virtual string SerializeRequestData(object data, RequestFormat format)
        {
            ...
            {
                case RequestFormat.Json:
                    return Serialization.JsonSerializeCommon(data);
                case RequestFormat.Xml:
                    return Serialization.XmlSerialize(data);
                case RequestFormat.Raw:
                    return data.ToString();
                default:
                    return Serialization.BinarySerialize(data);
            }
        }
        ...
        private void HandleHttpHeader(HttpWebRequest request)
        {
            ...
            while (iterator.MoveNext())
            {
                ...
                request.Headers[iterator.Current.Key] = val;
            }
        }
