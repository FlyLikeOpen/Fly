...
namespace Fly.Framework.Common
{
    ...
    internal static class EmitHelper
    {
            ...
            if (s_ModuleBuilder == null)
            {
                lock (s_ModuleSyncObj)
                {
                    if (s_ModuleBuilder == null)
                    {
                        ...
                        if (isPersist)
                        {
                            mdlBuilder = asmBuilder.DefineDynamicModule("Fly.DynamicType.Module_Impl", "ECommerce5.DynamicType.dll");
                        }
                        else
                        {
                            mdlBuilder = asmBuilder.DefineDynamicModule("Fly.DynamicType.Module_Impl");
                        }
        ...
        public static Type CreateType(Type interfaceType, IMethodEmit methodEmitter, bool isPersist, string prefix)
        {
            ...
            if (isPersist)
            {
                asmBuilder.Save("Fly.DynamicType.dll");
            }
            return tmp;
        }
