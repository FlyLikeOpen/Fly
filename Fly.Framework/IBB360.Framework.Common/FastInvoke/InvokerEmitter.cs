...
namespace Fly.Framework.Common
{
    internal class InvokerEmitter : IMethodEmit
    {
        ...
        {
            MethodInfo[] instanceMethodArray = GetMethodsExcludePropertyMethodAndOpenGenericTypeMethod(m_InvokedType, m_BindingFlags);
            MethodInfo[] staticMethodArray = GetMethodsExcludePropertyMethodAndOpenGenericTypeMethod(m_InvokedType, BindingFlags.Public | BindingFlags.Static);
            if ((instanceMethodArray == null || instanceMethodArray.Length <= 0) && (staticMethodArray == null || staticMethodArray.Length <= 0))
            {
                ilg.Emit(OpCodes.Ldstr, "类型'" + m_InvokedType.FullName + "'没有public的实例或静态方法.");
                ilg.Emit(OpCodes.Newobj, typeof(ApplicationException).GetConstructor(new Type[1] { typeof(string) }));
                ilg.Emit(OpCodes.Throw);
                return;
            }
            List<MethodInfo> allMethodList = new List<MethodInfo>(instanceMethodArray);
            allMethodList.AddRange(staticMethodArray);
            for (int ix=0; ix < allMethodList.Count; ix++)
            {
                MethodInfo method = allMethodList[ix];
                Label label1 = ilg.DefineLabel();
                ParameterInfo[] pInfoArray = method.GetParameters();
                ...
                ilg.Emit(OpCodes.Ldarg, 1);
                if (ix < instanceMethodArray.Length)
                {
                    ilg.Emit(OpCodes.Brfalse, label1);
                }
                else
            ...
            ilg.Emit(OpCodes.Ldloc_S, local_sb);
            ilg.Emit(OpCodes.Ldstr, "无法找到类型'" + m_InvokedType.FullName + "'的公开方法{0}(");
            ilg.Emit(OpCodes.Ldarg, 2);
